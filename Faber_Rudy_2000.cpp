...
static DefaultGUIModel::variable_t vars[] = {    
    { "Current Input (A)", "Current Input (A)", DefaultGUIModel::INPUT, }, // input(0)
    { "Voltage Output (V)", "Voltage Output (V)", DefaultGUIModel::OUTPUT, }, // output(0)
    { "Model Rate (Hz)", "Model Rate (Hz)", DefaultGUIModel::PARAMETER | DefaultGUIModel::INTEGER,}, // modelRate

    // Example parameters for manually changing conductance through GUI
    { "gNa", "Sodium Conductance", DefaultGUIModel::PARAMETER | DefaultGUIModel::DOUBLE,}, // gna

    // Example state for outputing another variable, such as the sodium current
    { "INa", "Sodium Current", DefaultGUIModel::STATE ,}, // I_Na
};
...
Faber_Rudy_2000::Faber_Rudy_2000(void) :
    DefaultGUIModel("Faber-Rudy 2000", ::vars, ::num_vars) {
...
void Faber_Rudy_2000::execute() {
    I_Inject = input(0) * -1 * 1e10; // Input in A for more compatibility with traditional current clamp input, Factor of 1e10 to replicate a whole cell capacitance in pF range
    ...
    output(0) = V / 1000; // Output in V for more compatability with traditional amplifier output
}
...
void Faber_Rudy_2000::update(DefaultGUIModel::update_flags_t flag) {
    ...
    case PAUSE: // Called when pause button is toggled
        // Model is reset to initial conditions when the modify button is clicked
    ...
    */
    ...
    for(z=0; z<20000; z++){
        ...
        if(Vx<-9.99 && Vx>-10.01)
            ...
            lkup[z][8] = lkup[z][7] * (1-exp(-(Vx+10)/6.24))/(0.035*(Vx+10)); //tau_d
        ...
        if (Vx<=0) 
            lkup[z][12] = -0.875*Vx + 12.0; //tau_g
        else 
            lkup[z][12] = 12.0; //tau_g
        lkup[z][13] = 1/(1+exp(-(Vx+14.0)/10.8)); //b_ss
        lkup[z][14] = 1/(1+exp((Vx+60.0)/5.6)); //g_ss
        lkup[z][15] = 1/(1+exp(-(Vx+21.5)/7.5)); //xr_ss
    
        if(Vx<-38.89&&Vx>-39.91)
            lkup[z][16] = 169;
        else
            lkup[z][16] = 1/(0.00138*(Vx+14.2)/(1-exp(-0.123*(Vx+14.2)))+0.00061*(Vx+38.9)/(exp(0.145*(Vx+38.9))-1)); //tau_xr
	
        lkup[z][17] = 1/(1+exp((Vx+9)/22.4)); //r_Kr
        lkup[z][18] = 1/(1+exp(-(Vx-1.5)/16.7)); //xs1_ss
	
        if(Vx<-29.99&&Vx>-30.01)
            lkup[z][19] = 417.9;
        else
            lkup[z][19] = 1/(0.0000719*(Vx+30)/(1-exp(-0.148*(Vx+30)))+0.000131*(Vx+30)/(exp(0.0687*(Vx+30))-1)); //tau_xs1
...
void Faber_Rudy_2000::solve() {
    ...
    if(V>-35 && dCa_ion_total_new > dCa_ion_total && flag==0) 
        {flag = 1; tcicr = 0;} 
